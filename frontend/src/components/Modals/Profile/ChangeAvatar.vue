<template>
  ...
  <modal-layout
    :bgShow="changeAvatar.bg"
    :modalShow="changeAvatar.modal"
    @close="close()">
...
export default {
  ...
  computed: {
    ...mapState('layout', ['changeAvatar']),
    ...mapState('user', ['user'])
  },
  methods: {
    ...mapMutations('layout', ['CHANGE_AVATAR_MODAL']),
    ...mapMutations('user', ['SET_USER_PROFILE']),
    ...
    close() {
      ...
      this.CHANGE_AVATAR_MODAL({ bg: false, modal: false, })
    },
    ...
    async selectFile(e) {
      ...
      try {
        this.status = 'loading';
        const response = await http.post('/api/v1/user-avatar', form, {
          headers: {
        ...
        if (response.data.data) {
          this.SET_USER_PROFILE({ user_info: response.data.data });
